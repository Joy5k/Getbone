...
const Wishlist = () => {
    ...
    const { data: wishlistItems = [] ,refetch,isLoading} = useQuery({
      ...
      console.log(wishlistItems,'got this data')
    const addQuantity = () => {
    ...
    const Items = wishlistItems.map(({img, title}: userProps)=>
        ...
        <td className="py-4 px-6">
            ...
            <button data-tooltip-target="tooltip-default" type="button" className="mb-5 font-medium btn text-red-700   p-2 rounded-md  focus:ring-4 focus:outline-none focus:ring-blue-300  text-lg px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><FaTrashAlt></FaTrashAlt></button>
            <div id="tooltip-default" role="tooltip" className="inline-block absolute invisible z-10 py-2 px-3 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 transition-opacity duration-300 tooltip dark:bg-gray-700">
                  Delete Item
            <div className="tooltip-arrow" data-popper-arrow>hello</div>
            </div>

       </td>
        <td className="py-0 px-0">
            <button className="btn text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-md  p-2 text-sm">Add to cart</button>
        </td>
    </tr>)
    return (
